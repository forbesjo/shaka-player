 ...
 * @private
 */
var parseRecords = function(recordData, recordCount) {
  var head = 0;
  var records = [];
  for (var i = 0; i < recordCount; i++) {
    ...
    var rawValue = recordRaw.subarray(offset, endIndex + offset + 1);
 ...
 * @private
 */
var parsePro = function(bytes) {
  var length = bytes[0] | bytes[1];
  var recordCount= bytes[3];

  var recordData = bytes.subarray(3);
  var records = parseRecords(recordData, recordCount);
...
shaka.dash.ContentProtection.getPlayReadyLicenseServerURL_ = function(element) {
  ...
  var parsedPro = parsePro(new Uint16Array(bytes.buffer));
