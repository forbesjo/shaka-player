...
shaka.dash.ContentProtection.getPlayReadyLicenseServerURL_ = function(element) {
  ...
  var bytes = shaka.util.Uint8ArrayUtils.fromBase64(proNode.innerHTML);
  ...
  for (var i = 0; i < numRecords; i++) {
    ...
    var recordData = bytes.buffer.slice(offset, offset + recordLength);
    var record = String.fromCharCode.apply(null, new Uint16Array(recordData));
    var xmlDoc = parser.parseFromString(record, 'application/xml').documentElement;

    // First try <LA_URL>
    var laurlNode = xmlDoc.getElementsByTagName('LA_URL')[0];
    if (laurlNode) {
    ...
    var luiurlNode = xmlDoc.getElementsByTagName('LUI_URL')[0];
    if (luiurlNode) {
      var luiurl = luiurlNode.childNodes[0].nodeValue;
