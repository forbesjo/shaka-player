...
shaka.media.TextEngine.makeCue = function(startTime, endTime, payload) {
  ...
  if (window.VTTCue &&
    ...
    return new window.VTTCue(startTime, endTime, payload);
  }
  ...
  var canUse3ArgsTextTrackCue = function() {
    try {
      return !!new window.TextTrackCue(1, 2, '');
    } catch (error) {
      return false;
    }
  };
  ...
  if (constructorLength == 3) {
    return new window.TextTrackCue(startTime, endTime, payload);
  ...
  // complaining.
    ...
    var id = startTime + '-' + endTime + '-' + payload;
    return new window['TextTrackCue'](id, startTime, endTime, payload);

  // IE10, IE11 and Edge returns TextTrackCue.length = 0 although it accepts 3
  // constructor arguments.
  } else if (canUse3ArgsTextTrackCue()) {
    return new window.TextTrackCue(startTime, endTime, payload);
  }
